...
const CalculationInputSchema = z
  .object({
    ...
    quantity: z.number().nonnegative().openapi({
      description: "Amount of the activity in its native inputUnit (see GET /v1/activities).",
      example: 100,
    }),
    factorId: z.string().optional().openapi({
      description:
...
const CalculationResultSchema = z
  .object({
    ...
    inputUnit: z.string().openapi({ description: "Unit of the original quantity" }),
    factor: z.string().openapi({ description: "Emission factor ID used for this result" }),
    result: z.object({
      kgCO2e: z.number().openapi({ description: "Calculated emissions in kg CO2 equivalent" }),
      unit: z.literal("kg"),
    }),
  })
...
const calculateRoute = createRoute({
  ...
  request: {
    body: { content: { "application/json": { schema: CalculateRequestSchema } } },
  },
  responses: {
    200: {
...
export function registerCalculateRoutes(app: OpenAPIHono) {
  app.openapi(calculateRoute, (c) => {
    const { inputs } = c.req.valid("json");

    const unknownActivities = inputs.filter((i) => !validActivityIds.has(i.activityId));
    if (unknownActivities.length > 0) {
      return c.json(
        { error: `Unknown activityId(s): ${unknownActivities.map((i) => i.activityId).join(", ")}. See GET /v1/activities.` },
        400 as const
      );
    }

    const unknownFactors = inputs.filter((i) => i.factorId && !validFactorIds.has(i.factorId));
    if (unknownFactors.length > 0) {
      return c.json(
        { error: `Unknown factorId(s): ${unknownFactors.map((i) => i.factorId).join(", ")}. See GET /v1/factors.` },
        400 as const
      );
    }

    return c.json(calculateFootprint(inputs), 200 as const);
  });
}
